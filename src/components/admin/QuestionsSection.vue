        ...
        <div v-if="type===1">
          ...
          <div class="row">
            <div class="col-md-3" v-for="(ch, i) in questionsNumberOfChoices[q-1]" :key="ch">
              <input class="form-control" v-model="questions.questions[q-1].choices[ch-1]" type="text" :placeholder="'Choice'+(i+1)"/>
            </div>
          </div>
        </div>
      </div>
...
export default {
  props: ['section'],
  created(){
    this.updateQuestionsModel('created');
  ...
  computed: {
    computed_questions() {
      return this.updateQuestionsModel('computed');
    }
  },
  watch: {
    ...
    methods: {
      updateQuestionsModel(method) {
        ...
        obj.questions = arr;
        if(method === 'created') this.questions = obj;
        if(method === 'computed') return obj;
      },
